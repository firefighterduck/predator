...
struct DfsItem {
  ...
  DfsItem(TBlock bb_) : bb(bb_), target(0) {}
};
...
void analyzeFnc(Fnc &fnc) {
  ...
  while (!dfsStack.empty()) {
    ...
    LS_DEBUG_MSG(1, edgeLoc,
                 "loop-closing edge detected: "
                     << bb->name() << " -> " << bbNext->name() << " (target #"
                     << target << ")");
#endif
